---
title: Migrating from MultiMC
sidebar:
  order: 3
---

## <img src="https://avatars2.githubusercontent.com/u/5411890" alt="MultiMC Logo" width=30px style="display: inline-block"/> In MultiMC

In order to transfer an instance, or multiple instances from MultiMC to Penguin Launcher, you must begin the process by opening MultiMC on your computer.

Now that MultiMC is open, navigate to the **Folders** tab located on the top menu bar, to the left of the Settings tab.

Left-click on the tab and select the **View Instance Folder** option. This should then open your system's file explorer application with the correct directory in view.

Now that the MultiMC instance directory is open, you may **select**, and then **copy** the instances that you wish to transfer.

## <img src="https://raw.githubusercontent.com/penguincre8te/PenguinLauncher/develop/program_info/org.penguinlauncher.PenguinLauncher.svg" alt="Penguin Launcher Logo" width=30px style="display: inline-block"> In Penguin Launcher

To complete the transfer process, you must now open Penguin Launcher.

As you did in MultiMC previously, navigate to the **Folders** tab located on the top menu bar, to the left of the Settings tab.

Left-click on the tab and select the **Instances** option. This should once again open your system's file explorer application with the correct directory in view.

Now that the Penguin Launcher instance directory is open, you may **paste** the instance(s) that you previously copied from the MultiMC directory.

_Optional: If you would like to transfer your pack icon selections, navigate up from the MultiMC instances folder to the main MultiMC folder. Do the same for Penguin Launcher. Copy the_ **icons** _folder from MultiMC to Penguin Launcher._

## Finishing up

Now, in **Penguin Launcher**, your instances should appear as they normally would in MultiMC.

If you have transferred instances across devices or packaging formats, you may need to change the version of **Java** that'll be used to launch Minecraft. You can learn how to do so on [this page](../installing-java).
